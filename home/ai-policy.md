...
This policy is designed to serve *your* best interests as a Data Analytics Fellow and as a future professional.

You are going to spend your career working alongside AI tools. Marcy is not going to pretend otherwise, and this policy is not an attempt to keep you away from them. Directing an AI tool well is a professional skill, and teaching it is part of what you are here for.

But a tool that writes analysis, code, or recommendations for you also hides whether you could have produced that work yourself. That is the problem this policy solves. A data professional who ships work they cannot explain, cannot debug, and cannot defend is not a fast or capable analyst — they are someone who has not noticed yet that they are stuck. The industry is full of people in exactly that position, and the fastest way to stand out is to avoid becoming one of them. Studies have shown time and time again that relying on AI too early eliminates understanding, which is precisely what you need to succeed in this field.

So this policy does not ask whether you used AI. It asks **what job you gave it.** That question has a different answer in your first weeks than it does later in the fellowship, and the policy changes with it.

{% hint style="info" %}
**Ownership without authorship.** The standard Marcy holds you to is this: you are responsible for every line of code, analysis, or written work you submit even if you did not personally type every line or phrase yourself. You will be asked to explain your work, defend your decisions, and fix it when it breaks. "The AI wrote that part" is never an answer, because it is always assumed that you still own the result.
{% endhint %}
...
| ------------------------------ | --------------------------------------------------------------------- | ------------------------------------------------------------- |
| What the AI does               | Explains, questions, quizzes, critiques, points you at the right idea | Writes code or analysis from a specification you wrote         |
| Who writes the work you submit | You do, by hand                                                       | The AI does, under your direction                             |
| What you are being graded on   | Whether you understand it                                             | Whether you specified it well, verified it, and can defend it |

Both modes are professional. They train different halves of the same job, and "Tutor mode" comes first because implementation can only happen once your personal understanding is strong enough to delegate work to someone or something else.

## Tutor Mode: The Baseline Standard

At the start of the fellowship, **AI does not produce code or analysis that ends up in your submission.** It can teach you anything you want to know about data work. It cannot do the thinking for you.

### The One-Sentence Test

> *Did any content in the file, notebook, or written response I am submitting arrive there because I copied it from a model, accepted a completion, or typed it out from a model's answer without understanding it first?*
...
Unlike Implementer mode, where you understand the concepts, have a plan, delegate implementation to AI, and then verify, in generator mode you are handing off the planning, thinking, and verification entirely to AI just to get the answer.

Generator mode shows up in a few recognizable shapes:

* **You ask for the artifact, not the input.** "Write a function that..." / "Fix this" / "Refactor this to be more Pythonic" — the request specifies the output you want, not the concept or skill you're missing.
* **You hand over the raw material and wait.** Pasting an error, a specification, or prompt and letting the model's response become your answer — even if you retype it by hand afterward. Retyping is not the same act as understanding.
* **You accept without producing.** Tab-completing a suggestion, copying a snippet, or taking a model's rewrite of your own code — the keystroke that puts work in your file happens before your understanding does.
* **You use the conversation as a shortcut around a skill, not toward one.** Debugging a traceback, learning a standard library tool, restructuring a loop, or interpreting output — anything the curriculum is trying to build reps in — gets skipped instead of practiced, even when the model's help felt collaborative in the moment.

Here are some examples of the differences between Generator mode and Tutor mode:

<details>

<summary><strong>You are stuck and do not know how to start</strong></summary>
...
<summary><strong>You have an error you do not understand</strong></summary>

**Generator mode:** "Fix this error." *(pastes the code and the traceback)*
...
Reading a traceback is a skill with a ceiling you will hit in a short period of practice, and then you will have it for the rest of your career. Handing the traceback to a model instead means you never start that practice.

</details>

<details>

<summary><strong>You suspect there is a built-in tool for what you are doing by hand</strong></summary>
...
You are allowed to learn that a built-in tool exists. You are not allowed to have the model swap it into your file.

</details>

<details>

<summary><strong>You finished and want to know if it is any good ⭐️</strong></summary>
...
This is one of the highest-value things you can do with an AI tool in Tutor Mode. A model that quizzes you finds the gap in your understanding faster than you will find it yourself.

</details>

<details>

<summary><strong>You do not understand something in the curriculum</strong></summary>

**Generator mode:** "Explain closures and show me an example." *(then copying the example)*
...
```
I am a Data Analytics Fellow learning Python and data tools. For this entire conversation, do not write code for me and do not fix my code. You may explain concepts, ask me questions, point me at documentation, tell me what to search for, and tell me when my reasoning is wrong. If I ask you to write code, remind me of this instruction instead. When I share code with you, ask me questions about it rather than rewriting it.
```

## Implementer Mode

In Implementer mode AI writes code or produces analysis for you, and the work *you* do becomes what happens on either side of what the AI does: the **specification** you write before, and the **verification** you do after.

What changes:

* **You write a specification first.** Before you generate anything, you write down what you are building, what files or notebooks it touches, what the interface looks like, and what "working" means.
* **You are accountable for every line.** You will be asked to explain any part of what you submitted. Work you cannot explain is work you did not own, and it is scored that way regardless of whether it runs.
* **Over-delivery is a defect.** You ask for a fix and get a refactor. You ask for one route and get a restructured analysis or pipeline. Accepting that silently is how you stop being the author of your own work. Catching it and rejecting it is part of the job.

This depends on the following workflow becoming a habit: research → plan → implement → test → refine → iterate.

### When the Mode Changes

The rule changes from "Tutor mode" to "Implementer mode," but you must be the judge for yourself when you are ready to make the transition. Being ready to make this transition means that you can:

* Explain why the generated code or analysis works — not just confirm that it runs.
* Predict what the solution needs.
* Identify defects or unnecessary complexity in AI-generated work.
...
If you are still developing your AI skills, you won't be held back from progressing through the program at any point, even if you stay in AI tutor mode. The AI mode that you are at determines *how* you will leverage AI to support you in assignments. The mode is what is adjusted, not your pace and not what you have access to.

We expect you to switch back and forth between modes as you develop mastery over new concepts. It is expected and encouraged that you start in AI tutor mode for every new concept and "graduate" to implementer mode as your progress.

Switching to "implementer mode" early to save time costs you the learning that comes from productive struggle. You may have assignments to show for it, but it will be difficult to explain your process or understanding of these concepts in an interview.

## Where AI Is Never Permitted

In any mode:

* During a graded check-in or live assessment.
